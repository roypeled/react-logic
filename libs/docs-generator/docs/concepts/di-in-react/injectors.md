...
4. On unmount, disposes the scope — runs `onDestroy` callbacks for every service it constructed locally.
...
The implicit root scope (no `<Injector>` wrapping a tree) handles auto-registered class injectables. You don't *need* an `<Injector>` to use the framework — it only matters when you want to bound service lifetime, override providers, or supply non-class tokens.
...
A child scope is **not** a copy of the parent — it just delegates lookups upward. Disposing the child doesn't touch services cached on the parent.

## Provider arrays

The `provide` prop accepts a mix of full provider objects and bare classes (sugar for `useClass: SameClass`):

```tsx
<Injector provide={[
  CounterStore,                                  // bare class — sugar
  { provide: API_URL, useValue: '/api' },        // value provider
  { provide: ApiClient, useFactory: () => new ApiClient(loadConfig()) },
]}>
```

The framework structurally compares the array on every render — same provider list (same tokens, same targets, same length) reuses the existing scope. Inline `provide={[...]}` literals don't churn services. Reordering or substituting a provider rebuilds the scope and disposes the old one.

## Avoid these injector smells

- **Wrapping every component in its own `<Injector>` "just in case."** A scope is overhead. Place it where lifetime actually matters.
